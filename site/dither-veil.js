// Adapted from React Bits DitherVeil by David Haz. See vendor/REACT-BITS-LICENSE.md.
...
void main() {
  ...
  vec3 sampleColor = textureLod(tImage, sampleUv, uLod).rgb;
  float backgroundKey = uKey * (1.0 - smoothstep(0.025, 0.12, distance(sampleColor, uMatte)));
  color = mix(color, uInk, max(backgroundKey, 1.0 - framed));
  vec3 backdrop = uInk;
...
export function mountDitherVeil(container, {
  src = DEFAULT_SRC,
  fit = 'contain',
  pattern = 'floyd',
  pixelSize = 2,
