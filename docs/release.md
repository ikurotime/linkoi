...
3. Run npm ci, npm run verify, npm run build:site, and npm run pack:local. Inspect both tarballs with npm pack --dry-run in each package.
...
5. Set package private flags to false only when ready to publish. Publish core first, then worker. npm publishing is deliberately not part of CI.
